...
class RhythmEngine {
    ...
    scheduleNote(beatNumber, time) {
        ...
        if (beatNumber % 4 === 0) {
            osc.frequency.value = 880; // Higher pitch on downbeat
        } else {
    ...
    getAccuracy() {
        ...
        // Find distance to closest beat time
        const timeSinceLast = current - this.lastBeatTime;
        const timeToNext = this.nextNoteTime - current;
        
        const dist = Math.min(timeSinceLast, timeToNext);
        const maxDist = spb / 2;
        
        const accuracy = 1.0 - (dist / maxDist);
        return accuracy;
    }
...
function updateBeatTracker() {
    if (!rhythm.audioCtx) return;
    const spb = 60.0 / rhythm.bpm;
    const current = rhythm.audioCtx.currentTime;
    
    // Calculate phase from 0 to 1 between beats
    let phase = ((current - rhythm.lastBeatTime) / spb) % 1.0;
    
    // Animate cursor from left marker to right marker and back
    // Or simpler: a repeating left-to-right that snaps back on beat
    beatCursor.style.left = `${phase * 100}%`;
    
    // Pulse markers if on beat
    if (phase < 0.1 || phase > 0.9) {
        document.getElementById('beat-marker-left').style.boxShadow = '0 0 20px #fff';
        document.getElementById('beat-marker-right').style.boxShadow = '0 0 20px #fff';
    } else {
        document.getElementById('beat-marker-left').style.boxShadow = '0 0 10px var(--neon-pink)';
        document.getElementById('beat-marker-right').style.boxShadow = '0 0 10px var(--neon-pink)';
    }
}
